...
import { errorFields, logWarn } from "../../../lib/logger";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return Response.json({ error: "Could not load liked videos." }, { status: 500 });
  }
}
  ...
  try {
    ...
    if (action === "unlike") {
      ...
      void updateCentroidsForPositiveEngagement(profileId, {
        ...
        logWarn("liked_video.centroid_update_failed", errorFields(error));
      });
    } else {
    ...
    return Response.json({ error: "Could not update liked videos." }, { status: 500 });
  }
}
